  ...
  app.post("/api/proofs/commitments", requireAuth, requireOrgContext, async (req: AuthRequest, res) => {
    try {
      ...
      if (typeof authenticityScore !== "number") {
        return res.status(400).json({ error: "authenticityScore must be a number" });
      }
...
async function generateProofHandler(req: AuthRequest, res: any) {
  try {
    ...
    try {
      ...
      res.json({
        proofRequest: { id: proofRequest.id, status: "completed", commitmentId, threshold: effectiveThreshold },
        proof: {
          id: proofResult.id,
...
async function verifyProofHandler(req: AuthRequest, res: any) {
  try {
    ...
    res.json({ valid: verification.valid, reason: verification.reason, proofId: proof.id, expiresAt: proof.expiresAt });
  } catch (error) {
...
async function publicVerifyHandler(req: any, res: any) {
  try {
    const { proofId } = req.body;
    if (!proofId) {
      return res.status(400).json({ error: "proofId is required" });
    ...
    res.json({ status: verification.valid ? "valid" : "invalid" });
  } catch (error) {
...
async function usageHandler(req: AuthRequest, res: any) {
  try {
    const now = new Date();
    const periodStart = new Date(now.getFullYear(), now.getMonth(), 1);
    const periodEnd = new Date(now.getFullYear(), now.getMonth() + 1, 0);
...
async function listProofsHandler(req: AuthRequest, res: any) {
  try {
...
async function getProofHandler(req: AuthRequest, res: any) {
  try {
